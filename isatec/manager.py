...
from .core import (CommunicableObjectBase, RestrictedStorableListBase,
                   SettingStorableObjectBase, StorableObject, iSATecObject)
from .defaultevent import (ColormapChangeEvent, ColorRegisterEvent,
                           ...
                           iSATecEvent, iSATecEventBinder, wxEVT_COLOR_REGISTER,
...
class IOManager(Singleton):
    ...
    def __init__(self, setting_file_path: str, core_mgr):
        ...
        default_path_list = [
            'isatec.objects.py',
            'isatec.container.py',
            'isatec.defaultevent.py',
            'isatec.defaultpeakfunction.py',
            'isatec.defaultdecodefunction.py',
            'isatec.defaultencodefunction.py',
            'isatec.defaultspectrumfunction.py',
            'isatec.defaultmappingfunction.py',
            'isatec.defaultpanel.py',
            'isatec.defaultmenuitem.py'
        ]
        exclude_class_list = [
            iSATecObject,
        ...
        iSATec_obj_list = self.__ImportObjectList(
            default_path_list + plugin_path_list, iSATecObject, exclude_class_list)

        storable_object_list = [Class() for Class in iSATec_obj_list if isclass(
            Class) and issubclass(Class, StorableObject) and Class not in exclude_class_list]
        storable_dict = {
            storable_object.__class__.__name__: storable_object for storable_object in storable_object_list}

        self.__temp_setting = DotNotationDict()
        self.__temp_setting.forced_setitem(STORABLE_OBJECT_DICT, storable_dict)

        dir_name = dirname(__file__)
        self.__setting = self.__ImportSetting(
            join(dir_name, setting_file_path))

        class_import_design = (
            (FUNCTION_CLASS_LIST, (PeakFunctionContainerBase, DecodeFunctionContainerBase,
                                   EncodeFunctionContainerBase, SpectrumFunctionContainerBase, MappingFunctionContainerBase,), False,),
            ...
            (EVENT_LIST, iSATecEventBinder, True,),
            (EVENT_RECEPTOR_CLASS_LIST, EventReceptorBase, False,),
            (SETTING_STORABLE_OBJECT_CLASS_LIST,
             SettingStorableObjectBase, False,),
        )

        for key, BaseClass, instanced in class_import_design:
            obj_list = []
            for iSATec_obj in iSATec_obj_list:
                if instanced:
                    if not isinstance(iSATec_obj, BaseClass):
                        continue
                else:
                    if not (isclass(iSATec_obj) and issubclass(iSATec_obj, BaseClass) or iSATec_obj in exclude_class_list):
                        continue

                obj_list.append(iSATec_obj)
    ...
    def GetSetting(self, *args) -> Any:
        ...
        key = args[0]
        default = self.GetInitialSetting(key, args[1]) if len(
            args) == 2 else self.GetInitialSetting(key)
    ...
    def GetInitialSetting(self, *args) -> dict:
        ...
        if len(args) != 0:
            ...
            value = self._initial_setting.get(
                DotChain(DEFAULT, key), default) if value is default else value
            return value
        else:
    ...
    def SaveSetting(self):
        ...
        * Have been used at least once in the application.
        ...
        app_setting = dumps(save_setting, ensure_ascii=False,
                            indent=4, sort_keys=True, cls=IOManager.iSATecJsonEncoder)
        dir_name = dirname(__file__)
    ...
    def SaveColorThemeList(self, color_theme_list: Iterable[Dict]):
        ...
        for color_theme in color_theme_list:
            ...
            contents = dumps(color_theme, ensure_ascii=False, indent=4,
                             sort_keys=True, cls=IOManager.iSATecJsonEncoder)
    ...
    def __ImportSetting(self, path):
        try:
            with open(path, mode='r', encoding='utf-8') as f:
                setting_dict = DotNotationDict(
                    load(f, object_hook=self.AsStorableObject))
            return setting_dict
        except FileNotFoundError as e:
    ...
    def __ImportObjectList(self, module_path_list, BaseClass, ExcludeClass=None):
        ...
        for module_path in module_path_list:
            ...
            try:
                ...
                LogError(
                    f'"{module_name}" module was not found.\n Please check the "setting.json"')
                continue
    ...
    def OpenProject(self, path: str) -> Project:
        ...
        contents = decompress(compressed_contents).decode(
            SAVE_ENCODING, 'replace')
    ...
    def SaveProject(self, project: Project):
        ...
        contents = dumps(project, separators=(',', ':'),
                         allow_nan=False, cls=IOManager.iSATecJsonEncoder)
        compressed_contents = compress(
            contents.encode(SAVE_ENCODING, 'replace'))
    ...
    class iSATecJsonEncoder(JSONEncoder):
        """Class for converting iSATec to json files
        """
...
class EventManager(Singleton):
    ...
    def GetEventList(self) -> Iterable[iSATecEventBinder]:
        """Get a list of registered events.

        :rtype: Iterable[iSATecEventBinder]
        """
        return self.__event_list

    def RegisterEventList(self, event_list: Iterable[iSATecEventBinder]):
        """Register a list of events.

        :type event_list: Iterable[Event]
        """
        if not HasValidElement(event_list, iSATecEventBinder):
            raise TypeError()
    ...
    def SendEventToReceptor(self, event: iSATecEvent):
        """Notifies the event receptor of the event.

        :type event: iSATecEvent
        """
        event.Skip()
        receptor_list = sorted(self.__event_receptor_list,
                               key=lambda x: self.GetReceptionOrder(x))
...
class MenubarManager(Singleton):
    ...
    def __init__(self, *args, **kw):
        ...
        self.__menu_item_dict = {EXIT_MENU_ITEM: MenuItem(
            id=ID_CLOSE, text=EXIT_MENU_ITEM, helpString='', kind=ITEM_NORMAL)}
        self.__menubar = None
    ...
    def RegisterMenuItemList(self, menu_item_list: Iterable[CustomMenuItemBase]):
        ...
        if any([not isinstance(menu_item, CustomMenuItemBase) for menu_item in menu_item_list]):
            raise TypeError(
                'Element of "menu_item_list" should be instance of "CustomMenuItemBase".')
    ...
    def __CreateMenu(self, design):
        ...
        for key in menu_item_keys:
            ...
            if isinstance(menu_item, ShowPanelMenuItem):
                ...
                shown = self.__io_mgr.GetSetting(
                    DotChain(panel.__class__.__name__, SHOW), False)
    ...
    def OnEvent(self, event):
        ...
        if event_type == wxEVT_LAUNCH:
            ...
            self.__core_mgr.Get(EVENT_MANAGER).RegisterEventReceptorList(
                self.GetPeakMenuItemList())
...
class PanelManager(Singleton, AuiManager):
    ...
    def __init__(self, *args, **kw):
        ...
        """
        super().__init__(agwFlags=AUI_MGR_ALLOW_FLOATING | AUI_MGR_ALLOW_ACTIVE_PANE |
                         AUI_MGR_TRANSPARENT_DRAG | AUI_MGR_LIVE_RESIZE)
        self.__core_mgr = kw['core_manager']
        self.__io_mgr = kw['io_manager']
        self.__id = NewIdRef()
    ...
    def RegisterPanelList(self, panel_list: Iterable[PanelBase], layout: Optional[str] = None):
        ...
        for panel in panel_list:
            ...
            if (key := Camel2Pascal(panel.__class__.__name__)) in panel_info_dict:
                ...
                aui_info.MinSize(panel_info['minw'], panel_info['minh'])
                aui_info.MaxSize(panel_info['maxw'], panel_info['maxh'])
                aui_info.FloatingSize(
                    (panel_info['floatw'], panel_info['floath']))
    ...
    def __CreateAuiPaneInfo(self, panel) -> AuiPaneInfo:
        ...
        aui_info = AuiPaneInfo().MaximizeButton().PinButton(
        ).CloseButton().NotebookDockable(False).Hide().Caption(name)

        is_shown = self.__core_mgr.Get(
            f'{panel.__class__.__name__}.{SHOW}', True)
        if not is_shown:
    ...
    def LoadLayout(self, name: str):
        ...
        event = LayoutChangeEvent(
            self.__selected_layout, prev_layout, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def RegisterCurrentLayout(self, name: str):
        ...
        event = LayoutRegisterEvent(
            layout_list, prev_layout_list, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def __SendPanelSelectionChangeEvent(self, panel):
        ...
        event = PanelSelectionChangeEvent(
            panel, self.__selected_panel, self.__id)
    ...
    def OnEvent(self, event):
        ...
        if event_type == wxEVT_PANEL_SELECTION_CHANGE:
            ...
            self.Update()
            self.__panel_dict = {
                panel.__class__.__name__: panel for panel in self.GetPanelList()}

        elif event_type == wxEVT_EXIT:
            # notebook_count = 0
            for panel in self.GetPanelList():
                self.__io_mgr.SetSetting(
                    DotChain(panel.__class__.__name__, SHOW), panel.Shown)

            self.__io_mgr.SetSetting(
                SELECTED_LAYOUT, (self.__selected_layout, self.SavePerspective()))
...
class FunctionManager(Singleton):
    ...
    def __RegisterFunctionList(self, function_list, is_register):
        ...
        event_design = [
            (PeakFunctionRegisterEvent, PeakFunctionDeregisterEvent,
             peak_func_list, prev_peak_func_list),
            (EncodeFunctionRegisterEvent, EncodeFunctionDeregisterEvent,
             encode_func_list, prev_encode_func_list),
            (DecodeFunctionRegisterEvent, DecodeFunctionDeregisterEvent,
             decode_func_list, prev_decode_func_list),
            (SpectrumFunctionRegisterEvent, SpectrumFunctionDeregisterEvent,
             spectrum_func_list, prev_spectrum_func_list),
            (MappingFunctionRegisterEvent, MappingFunctionDeregisterEvent,
             mapping_func_list, prev_mapping_func_list),
        ]
    ...
    def SelectEncodeFunction(self, function: EncodeFunctionContainerBase):
        ...
        event = EncodeFunctionSelectEvent(
            function, prev_function, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def SelectDecodeFunction(self, function: DecodeFunctionContainerBase):
        ...
        event = DecodeFunctionSelectEvent(
            function, prev_function, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def SelectSpectrumFunctionList(self, function_list: Iterable[SpectrumFunctionContainerBase]):
        ...
        event = SpectrumFunctionListSelectEvent(
            func_list, prev_func_list, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def SelectMappingFunction(self, function: MappingFunctionContainerBase):
        ...
        event = MappingFunctionSelectEvent(
            function, prev_function, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def __FormatFunctionList(self, function_list):
        if isinstance(function_list, str):
            ...
            if all([isinstance(function, FunctionContainerBase) for function in function_list]):
                ...
                function_list = [self.GetFunction(
                    element) for element in function_list]
            else:
                raise TypeError()
    ...
    def __FormatPresetList(self, preset_list):
        if isinstance(preset_list, str):
            ...
            if all([isinstance(preset, Preset) for preset in preset_list]):
                ...
                preset_list = [self.GetPreset(preset)
                               for preset in preset_list]
            else:
                TypeError()
...
class ProjectManager(Singleton):
    ...
    def OpenProject(self, path: str):
        ...
        event = ProjectOpenEvent(
            data_list, path, peak_type, note, experimental_date, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def SaveProject(self, project: Project = None) -> bool:
        ...
        event = ProjectSaveEvent(
            path, data_list, peak_type, note, experimental_date, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def SetProjectMemo(self, experimental_date: date, note: str):
        ...
        event = ProjectMemoChangeEvent(
            experimental_date, prev_date, note, prev_note)
        self.__core_mgr.SendEvent(event)
    ...
    def AskProjectSaving(self) -> bool:
        ...
        if self.IsProjectStarted() and not self.IsProjectSaved():
            ...
            if id_ == ID_CANCEL:
                ...
                is_saved = self.__core_mgr.Get(
                    MENUBAR_MANAGER).ExecuteMenuFunction(SAVE_MENU_ITEM)
...
class PeakManager(Singleton):
    ...
    def RegisterPeakTypeList(self, peak_type_list: Union[PeakType, Iterable[PeakType]]):
        ...
        event = PeakTypeRegisterEvent(
            peak_type_list, prev_peak_type_list, self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def OnEvent(self, event):
        ...
        if event_type == wxEVT_PROJECT_NEW:
            ...
            peak_type = event.GetPeakType()
            self.__GetProject().SetPeakType(peak_type)
            self.__core_mgr.Get(MENUBAR_MANAGER).CheckMenuItem(
                peak_type.GetName())
...
class DataManager(Singleton):
    ...
    def GetDataList(self, index_list: Iterable[int] = None) -> List[DataContainer]:
        ...
        data_list = data_list if index_list is None else [
            data_list[index] for index in index_list]
    ...
    def SetDataList(self, index_list: Iterable[int], data_list: Iterable[DataContainer]):
        ...
        x_changed_list = y_changed_list = bg_changed_list = peaks_changed_list = recipe_changed_list = msg_changed_list = [
            True] * len(data_list)

        event = DataContentsChangeEvent(index_list, data_list, x_changed_list, y_changed_list,
                                        bg_changed_list, peaks_changed_list, recipe_changed_list, msg_changed_list, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def ApplyRecipe(self, recipe: Recipe, index_list: Iterable[int] = None):
        ...
        event = DataContentsChangeEvent(index_list, data_list, recipe_changed_list=[
                                        True] * len(index_list), id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def Select(self, main_selection: Union[int, bool, None] = False, selection: Union[Iterable[int], bool, None] = False, operand: Optional[str] = None):
        ...
        event = DataSelectionChangeEvent(self.__main_selection[1], self.__main_selection[0], list(
            self.__selection[1]), list(self.__selection[0]))
        self.__core_mgr.SendEvent(event)
    ...
    def ExecuteSpectrumFunction(self, index_list: Optional[Iterable[int]] = None):
        ...
        """
        data_list = self.__GetDataList()
        index_list = list(range(self.GetDataSize())
                          ) if index_list is None else index_list
        ...
        for index in index_list:
            ...
            for n in range(len(new_recipe)):
                ...
                data.Append(Spectrum(x, y, bg, peaks),
                            new_recipe, new_success_list, msg)

        index_list = self.GetIndexList(data_list)
        event = DataContentsChangeEvent(index_list, data_list, x_changed_list, y_changed_list, bg_changed_list, peaks_changed_list, [
                                        True] * len(index_list), msg_changed_list, id=self.__id)
        self.__core_mgr.SendEvent(event)
...
class SpectrumManager(Singleton):
    ...
    def Draw(self, need_bg=True, need_peaks=False, multi_draw_alpha=0.0):
        ...
        spectrum_panel = self.__core_mgr.Get(
            PANEL_MANAGER).GetPanel(SPECTRUM_PANEL)
        ...
        if main_selection is not None:
            ...
            if need_bg:
                ...
                if len(bg) != 0:
                    bg_line_list.append(
                        Line2D(x, bg, ls='', marker='.', ms=3, c='gray'))
        ...
        if multi_draw_alpha > 0:
            for i in selection:
                ...
                line_list.append(
                    Line2D(x, y, alpha=multi_draw_alpha, ls='', marker='.', ms=3, c='gray'))
...
class EncodeManager(Singleton):
    ...
    def SelectEncoding(self, encoding: Union[str, ChoiceContainer]):
        ...
        event = EncodeEvent(encoding=encoding,
                            previous_encoding=prev_encoding, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def SelectDelimiter(self, delimiter: Union[str, ChoiceContainer]):
        ...
        event = EncodeEvent(delimiter=delimiter,
                            previous_delimiter=prev_delimiter, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def GetEncodeFunctionNameContainer(self) -> ChoiceContainer:
        ...
        :rtype: ChoiceContainer
        """
        selected_func = self.__core_mgr.Get(
            FUNCTION_MANAGER).GetSelectedEncodeFunction()
        func_list = self.__core_mgr.Get(
            FUNCTION_MANAGER).GetEncodeFunctionList()
...
class DecodeManager(Singleton):
    ...
    def SelectEncoding(self, encoding: Union[str, ChoiceContainer]):
        ...
        prev_encoding = self.__encoding
        self.__encoding = encoding
        event = DecodeEvent(encoding=encoding,
                            previous_encoding=prev_encoding, id=self.__id)
        self.__core_mgr.SendEvent(event)
    ...
    def GetDecodeFunctionNameContainer(self) -> ChoiceContainer:
        ...
        """
        selected_func = self.__core_mgr.Get(
            FUNCTION_MANAGER).GetSelectedDecodeFunction()
        func_list = self.__core_mgr.Get(
            FUNCTION_MANAGER).GetDecodeFunctionList()
        selected_name = selected_func.__class__.__name__
        func_name_list = [func.__class__.__name__ for func in func_list]
        return ChoiceContainer(selected_name, func_name_list)
...
class ColorManager(Singleton):
    ...
    def RegisterColorThemeList(self, color_theme_list: Union[List, Tuple, Dict]):
        ...
        event = ColorRegisterEvent(
            color_theme_list, prev_color_theme_list, id=self.__id)
        self.__core_mgr.SendEvent(event)
