...
class iSATecObject:
    """The base class for all iSATec objects, mainly used to distinguish which objects should be managed by the manager.
    """
    pass


class StorableObject(iSATecObject, metaclass=ABCMeta):
...
class CommunicableObjectBase(iSATecObject):
    """
...
class SettingStorableObjectBase(Singleton, iSATecObject):
    """Inherit this class when you want iSATec to save non-default settings until after the application is launched.
    """
...
class ChameleonWidgetBase(iSATecObject):
    @classmethod
    def SetStyle(cls):
        pass
